...
[iCarousel](https://github.com/nicklockwood/iCarouse)是一个类，它继承自UIView。用于简化实现各种类型的旋转木马(分页滚动视图)iPhone、iPad和Mac OS。iCarousel实现一些常见的影响如圆柱、平面式的旋转木马。经过 iCarousel类的封装，使iCarousel类的使用方式类似于UITableView的使用，每一个界面类似于一个单元格。 iCarousel类为我们提供了十几种分页效果，我们还可以使用 iCarousel类中的 (BOOL) vertical属性设置显示效果是垂直显示还是水平显示。
...
// 设置单元格显示是水平或竖直，默认NO（水平）
@property (nonatomic, assign, getter = isVertical) BOOL vertical;
// 设置是否启动旋转木马效果, readonly 外部无法直接设置，默认为NO
...
// setDelegate 时  setNeedsLayout  重新布局
...
#endif
        // 每秒执行60次step方法
        // step的主要怕任务是 通过计算下一步的事物执行流程
        // 最后会执行didScroll来执行最后的视图变更
...
/*
 关键流程
 ----- 检查视图index是否改变
 ----- loadUnloadViews 新旧视图的更新和维护
 ----- transformItemView 视图变更过程
 ----- 通知代理执行相关代理方法进行重新绘制或数据变更
 ----- 更新控件相关数据参数
 */
- (void)didScroll
{
